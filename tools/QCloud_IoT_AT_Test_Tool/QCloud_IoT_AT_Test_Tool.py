...
def gen_template_update_msg():
    ...
    motor1 = random.randint(0, 2)
    power_switch = random.randint(0, 1)
    position = random.randint(1, 10000)
    print(">>> update data >>> brightness:", brightness, "color:", color, "power_switch:", power_switch, "motor1", motor1)
    IEUpdateMsg = '''{
        "type": "update",
        "state": {
            "reported": {
                ...
                "text_test":"test-long-text-report-in-json-position-%d"
            }
        },
        "version":%d,
        "clientToken": "clientToken-%d" }''' \
                  % (brightness, color, power_switch, motor1, position, position, 0, int(time.time()*10)%1000)
    return IEUpdateMsg.replace(' ', '').replace('\n', '')
...
def gen_event_post_msg():
    ...
    return IEEventMsg.replace(' ', '').replace('\n', '')
        ...
        while send_cnt < loop_cnt:
            ...
            while True:
                try:
                    ...
                    recv_timeout_cnt = 0
                    if recv_payload.replace(' ', '').replace('\n', '') == loop_test_last_msg:
                        recv_cnt += 1
...
class IoTExplorerATTest(IoTBaseATCmd):
    ...
    def do_loop_test(self, loop_cnt):
        ...
        while test_cnt < loop_cnt:
            logging.info("------IoT Explorer template/event loop test cnt: %d" % test_cnt)
            if self.publish_template_msg(gen_template_update_msg(), qos=1):
